...
//	}
	...
	private int rowTillEndOfPage(int numLine) {
		if (numLine <= EngineVP2G.firstPageColumnQty)
			return (EngineVP2G.firstPageColumnQty - numLine+1);
		else
			return (EngineVP2G.notFirstPageColumnQty - ((numLine - EngineVP2G.firstPageColumnQty+1) % EngineVP2G.notFirstPageColumnQty));
	} 
...
//			EngineVP2G.getSectionByID(section.getID());
		...
		int startLine = EngineVP2G.numOfCurrLine;
...
//				addEmptyRow(1);
			...
			startLine = EngineVP2G.numOfCurrLine;
...
//				System.out.println("GOT < OR >!!!!");
	...
	static String getNonBreakableSymbols(String inStr) {
		int index = -1;
		if ((index = (Arrays.asList(EngineVP2G.nonbreakablePlaneArray)).indexOf(inStr)) >= 0) {
			return EngineVP2G.nonbreakableArray[index].replaceAll("[^<>]", ""); 
		}
		else return "";
	}
